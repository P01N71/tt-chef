...
const Step3Save = () => {
    ...
    const [shelfTag, setShelfTag] = useState('26년도 가을학기'); // 2학기 태그로 기본값 변경
    ...
    const renderTimetableOnly = () => (
        <div className="w-full bg-white p-8 rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
            <div className="text-center mb-8 border-b pb-6 border-slate-100">
                <h3 className="text-3xl font-black text-slate-900 mb-2 tracking-tight">2026 가을학기 시간표</h3>
    ...
    const exportToImage = useCallback(async () => {
        ...
        try {
            ...
            link.download = `2026_새학기_시간표.png`;
            link.href = dataUrl;
            link.click();
        } catch (err) {
    ...
    // ★ [수정됨] 2학기 개강일(8월 24일) 및 16주 반복 적용
    const handleExportCalendar = () => {
        ...
        const semesterMonth = 8; // 8월 시작

        // 2026년 8월 24일(월) 개강 주간에 맞춘 요일-날짜 매핑
        ...
        safeSchedule.forEach((course) => {
            ...
            course.times.forEach((time) => {
                const firstDate = firstWeekDates[time.day] || 24;
                ...
                events.push({
                    title: course.name,
                    location: course.room || '', // 오직 장소 정보(E#-###)만 삽입
        ...
        createEvents(events, (error, value) => {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', '2026_2학기_시간표.ics');
